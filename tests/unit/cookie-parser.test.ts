...
describe("parseSetCookieHeader", () => {
  ...
  it("Expires属性をパースする", () => {
    const result = parseSetCookieHeader("id=1; Expires=Mon, 01 Jan 2026 00:00:00 GMT");
...
describe("getCookieSecurityWarnings", () => {
  ...
  it("Secureがない場合に警告を出す", () => {
    ...
    const hasSecureWarning = warnings.some((w) => w.message.includes("Secure"));
  ...
  it("HttpOnlyがない場合に警告を出す", () => {
    ...
    const warnings = getCookieSecurityWarnings(attrs);
    const hasHttpOnlyWarning = warnings.some((w) => w.message.includes("HttpOnly"));
  ...
  it("SameSiteがない場合に警告を出す", () => {
    ...
    const warnings = getCookieSecurityWarnings(attrs);
    const hasSameSiteWarning = warnings.some((w) => w.message.includes("SameSite"));
