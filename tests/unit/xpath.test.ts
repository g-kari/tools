    ...
    return {
      querySelector: (selector: string) =>
        selector === 'parsererror'
          ? { textContent: 'XML parse error: invalid structure' }
          : null,
    };
  }
}
...
function makeNodesetResult(nodes: object[], ordered = false) {
  ...
  return {
    ...
    iterateNext: vi.fn().mockImplementation(() =>
      idx < nodes.length ? nodes[idx++] : null,
    ),
  };
}
...
describe('evaluateXPath', () => {
  ...
  describe('ブラウザ環境なし (Node.js)', () => {
    it('windowが未定義の場合はエラーを返す', () => {
      ...
      expect(['error', 'nodeset', 'string', 'number', 'boolean']).toContain(
        result.type,
      );
    });
  });
...
describe('XPATH_EXAMPLES', () => {
  ...
  it('count() 関数を使用する例が含まれる', () => {
    const hasCount = XPATH_EXAMPLES.some((ex) =>
      ex.expression.includes('count('),
    );
    expect(hasCount).toBe(true);
  });

  it('contains() 関数を使用する例が含まれる', () => {
    const hasContains = XPATH_EXAMPLES.some((ex) =>
      ex.expression.includes('contains('),
    );
