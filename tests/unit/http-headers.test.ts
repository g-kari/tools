...
import { HTTP_HEADERS, filterHeaders, type HttpHeader } from "../../app/routes/http-headers";
...
describe("HTTP_HEADERS データ", () => {
  ...
  it("全ヘッダーに必須フィールドが存在する", () => {
    for (const header of HTTP_HEADERS) {
      ...
      expect(header.description, `${header.name} に description がない`).toBeTruthy();
      expect(header.example, `${header.name} に example がない`).toBeTruthy();
      expect(typeof header.security, `${header.name} の security が boolean でない`).toBe(
        "boolean",
      );
    }
  });
  ...
  it("Content-Security-Policy ヘッダーがセキュリティ関連として分類されている", () => {
    const header = HTTP_HEADERS.find((h) => h.name === "Content-Security-Policy");
    expect(header).toBeDefined();
    expect(header!.security).toBe(true);
  ...
  it("Strict-Transport-Security ヘッダーが response カテゴリでセキュリティ関連", () => {
    const header = HTTP_HEADERS.find((h) => h.name === "Strict-Transport-Security");
    expect(header).toBeDefined();
    expect(header!.category).toBe("response");
    expect(header!.security).toBe(true);
  });
