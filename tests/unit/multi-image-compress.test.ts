 ...
 */
function createMockCompressItem(override: Partial<CompressItem> = {}): CompressItem {
  return {
...
describe("filterDuplicateFiles", () => {
  it("重複なしの場合は全ファイルを返す", () => {
    const existing = [createMockCompressItem({ originalFile: createMockFile("a.jpg", 100) })];
    const newFiles = [createMockFile("b.jpg", 200), createMockFile("c.png", 300)];
  ...
  it("同名・同サイズのファイルは除外される", () => {
    const existing = [createMockCompressItem({ originalFile: createMockFile("photo.jpg", 1000) })];
    const newFiles = [
