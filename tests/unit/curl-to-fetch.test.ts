...
describe('parseCurl', () => {
  ...
  it('-d でボディを解析してメソッドをPOSTにする', () => {
    const { parsed } = parseCurl(
      "curl 'https://api.example.com' -d '{\"name\":\"test\"}'"
    );
  ...
  it('--data-raw でボディを解析する', () => {
    const { parsed } = parseCurl(
      "curl --data-raw 'test data' https://api.example.com"
    );
  ...
  it('--json でContent-TypeとAcceptを設定する', () => {
    const { parsed } = parseCurl(
      "curl --json '{\"key\":\"val\"}' https://api.example.com"
    );
  ...
  it('--oauth2-bearer でAuthorizationヘッダーを設定する', () => {
    const { parsed } = parseCurl(
      'curl --oauth2-bearer mytoken https://api.example.com'
    );
  ...
  it('-d があってもメソッドを明示的に指定した場合はそちらを優先する', () => {
    const { parsed } = parseCurl(
      "curl -X PATCH -d '{\"k\":\"v\"}' https://api.example.com"
    );
  ...
  it('curlなしで入力しても解析できる', () => {
    const { parsed } = parseCurl(
      "https://api.example.com -H 'Accept: application/json'"
    );
