...
describe("convertHtmlToJsx", () => {
  describe("class属性の変換", () => {
    ...
    it("複数のclass属性をすべて変換する", () => {
      const { output } = convertHtmlToJsx('<div class="a"><span class="b"></span></div>');
  ...
  describe("style属性の変換", () => {
    ...
    it("複数のCSSプロパティを変換する", () => {
      const { output } = convertHtmlToJsx('<p style="font-size: 14px; margin-top: 8px">');
  ...
  describe("変更点リストの集計", () => {
    ...
    it("複数の変換が行われた場合は各エントリを返す", () => {
      const { changes } = convertHtmlToJsx('<div class="foo" onclick="fn()"><br></div>');
    ...
    it("件数が正しく集計される", () => {
      const { changes } = convertHtmlToJsx('<div class="a"><span class="b"></span></div>');
  ...
  describe("複合テスト", () => {
    it("複数の変換ルールが同時に適用される", () => {
      const html = '<div class="container" onclick="fn()"><br><img src="x.png" alt="img"></div>';
