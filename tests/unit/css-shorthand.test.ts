...
describe("splitCssValues", () => {
  ...
  it("4値を正しく分割する", () => {
    expect(splitCssValues("10px 20px 30px 40px")).toEqual(["10px", "20px", "30px", "40px"]);
  });

  it("括弧内のスペースを区切りとして扱わない", () => {
    expect(splitCssValues("calc(100% - 20px) auto")).toEqual(["calc(100% - 20px)", "auto"]);
  });
...
describe("collapseShorthand - overflow", () => {
  it("x と y が同じ: 1値に圧縮", () => {
    expect(collapseShorthand("overflow", { "overflow-x": "hidden", "overflow-y": "hidden" })).toBe(
      "hidden",
    );
  });

  it("x と y が異なる: 2値のまま", () => {
    expect(collapseShorthand("overflow", { "overflow-x": "hidden", "overflow-y": "auto" })).toBe(
      "hidden auto",
    );
  });
});
...
describe("collapseShorthand - gap", () => {
  it("row と column が同じ: 1値に圧縮", () => {
    expect(collapseShorthand("gap", { "row-gap": "16px", "column-gap": "16px" })).toBe("16px");
  });

  it("row と column が異なる: 2値のまま", () => {
    expect(collapseShorthand("gap", { "row-gap": "16px", "column-gap": "24px" })).toBe("16px 24px");
  });
});
