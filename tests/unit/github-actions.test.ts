...
describe("generateWorkflow", () => {
  ...
  it("グローバル環境変数が正しく出力されること", () => {
    const config = defaultWorkflowConfig();
    config.globalEnv = [{ key: "TZ", value: "Asia/Tokyo" }];
    const result = generateWorkflow(config);
    expect(result).toContain("env:");
...
describe("GITHUB_ACTIONS_TEMPLATES", () => {
  ...
  it("Node.js CI テンプレートが正しいYAMLを生成すること", () => {
    const template = GITHUB_ACTIONS_TEMPLATES.find((t) => t.label === "Node.js CI");
  ...
  it("Python CI テンプレートが正しいYAMLを生成すること", () => {
    const template = GITHUB_ACTIONS_TEMPLATES.find((t) => t.label === "Python CI");
    expect(template).toBeDefined();
    if (!template) return;
  ...
  it("Docker Build & Push テンプレートが正しいYAMLを生成すること", () => {
    const template = GITHUB_ACTIONS_TEMPLATES.find((t) => t.label === "Docker Build & Push");
    expect(template).toBeDefined();
    if (!template) return;
  ...
  it("Cloudflare Workers Deploy テンプレートが正しいYAMLを生成すること", () => {
    const template = GITHUB_ACTIONS_TEMPLATES.find((t) => t.label === "Cloudflare Workers Deploy");
    expect(template).toBeDefined();
    if (!template) return;
...
describe("defaultWorkflowConfig", () => {
  ...
  it("デフォルト設定のジョブにチェックアウトステップが含まれること", () => {
    const config = defaultWorkflowConfig();
    const checkoutStep = config.jobs[0].steps.find((s) => s.type === "checkout");
