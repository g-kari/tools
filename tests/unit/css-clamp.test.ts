...
describe('calculateFluid', () => {
  ...
  it('minViewport でクランプされていない', () => {
    const result = calculateFluid(DEFAULT_FLUID_CONFIG);
    const minPt = result.points.find(
      (p) => p.viewport === DEFAULT_FLUID_CONFIG.minViewport
    );
    if (minPt) {
  ...
  it('maxViewport でクランプされていない', () => {
    const result = calculateFluid(DEFAULT_FLUID_CONFIG);
    const maxPt = result.points.find(
      (p) => p.viewport === DEFAULT_FLUID_CONFIG.maxViewport
    );
    if (maxPt) {
