...
describe("SEOメタタグ生成", () => {
  describe("escapeHtmlAttr", () => {
    ...
    it("複数の特殊文字を同時にエスケープする", () => {
      expect(escapeHtmlAttr('<a href="?a=1&b=2">')).toBe("&lt;a href=&quot;?a=1&amp;b=2&quot;&gt;");
    });
  });
  ...
  describe("generateBasicMetaTags", () => {
    ...
    it("タイトルのHTMLエスケープが正しく行われる", () => {
      ...
      const tags = generateBasicMetaTags(data);
      expect(tags).toContain("  <title>A &lt;B&gt; &amp; &quot;C&quot;</title>");
    });
  ...
  describe("generateOgTags", () => {
    it("全フィールド入力時に正しいOGタグを生成する", () => {
      ...
      expect(tags).toContain('  <meta property="og:image" content="https://example.com/ogp.png">');
  ...
  describe("generateTwitterTags", () => {
    it("全フィールド入力時に正しいTwitterタグを生成する", () => {
      ...
      expect(tags).toContain('  <meta name="twitter:title" content="Twitterタイトル">');
      expect(tags).toContain('  <meta name="twitter:description" content="Twitter説明文">');
      expect(tags).toContain(
