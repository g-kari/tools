...
test.describe("テトリス (/tetris)", () => {
  ...
  test("ゲームスタートボタンが表示される", async ({ page }) => {
    await expect(
      page.getByRole("button", { name: "ゲームスタート" }).first()
    ).toBeVisible();
  });
  ...
  test("メインキャンバスが表示される", async ({ page }) => {
    await expect(
      page.getByRole("application", { name: "テトリスゲームボード" })
    ).toBeVisible();
  });
  ...
  test("ナビゲーションのゲームカテゴリにテトリスが含まれる", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("menuitem", { name: "テトリス" })
    ).toBeVisible();
  });
});
