...
test.describe("テキスト ↔ バイナリ変換ページ", () => {
  ...
  test("ページが正しく表示される", async ({ page }) => {
    await expect(page.getByRole("heading", { name: /テキスト.*バイナリ変換/ })).toBeVisible();
  });

  test("モード切替ボタンが表示される", async ({ page }) => {
    await expect(page.getByRole("button", { name: /テキスト → バイナリ/ })).toBeVisible();
    await expect(page.getByRole("button", { name: /バイナリ → テキスト/ })).toBeVisible();
  });
  ...
  test("バイナリ → テキストモードに切り替えられる", async ({ page }) => {
    ...
    await page.locator("#text-binary-input").fill("01000001");
  ...
  test("結果をコピーボタンが出力がある場合に表示される", async ({ page }) => {
    await expect(page.getByRole("button", { name: /結果をコピー/ })).not.toBeVisible();
    await page.locator("#text-binary-input").fill("A");
  ...
  test("区切り文字をコンマに切り替えられる", async ({ page }) => {
    ...
    await expect(page.locator("#text-binary-output")).toHaveValue("01000001,01000010");
  });
});
