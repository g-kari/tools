...
test.describe('プレイフェア暗号ページ', () => {
  ...
  test('復号化モードに切り替えられる', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: '復号化' })).toHaveAttribute('aria-pressed', 'true');
    await expect(page.getByRole('button', { name: '暗号化' })).toHaveAttribute('aria-pressed', 'false');
  });
});
