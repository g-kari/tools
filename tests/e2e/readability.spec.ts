...
test.describe('可読性スコア分析ページ', () => {
  ...
  test('ページが正しく表示される', async ({ page }) => {
    await expect(
      page.getByRole('heading', { name: /可読性スコア分析/ })
    ).toBeVisible();
  });
  ...
  test('初期状態では空状態メッセージが表示される', async ({ page }) => {
    await expect(
      page.getByText('テキストを入力すると可読性スコアが表示されます')
    ).toBeVisible();
  });
  ...
  test('英語テキストで言語バッジが English を表示', async ({ page }) => {
    await page.locator('textarea').fill(
      'Hello world. This is a test sentence for the readability tool.'
    );
  ...
  test('日本語テキストを入力すると難易度スコアが表示される', async ({ page }) => {
    await page.locator('textarea').fill(
      '本日は晴天なり。日本語のテキスト解析を行うツールです。漢字の密度を計算します。'
    );
  ...
  test('日本語テキストで言語バッジが日本語を表示', async ({ page }) => {
    await page.locator('textarea').fill(
      'これはテストです。日本語のテキストを解析します。漢字密度を測定します。'
    );
