...
test.describe('色温度変換ページ', () => {
  ...
  test('HEX値が表示される', async ({ page }) => {
    const hexValue = page.locator('.ct-value-row').filter({ hasText: 'HEX' }).locator('.ct-value-code');
    await expect(hexValue).toBeVisible();
    await expect(hexValue).toContainText('#');
  });

  test('RGB値が表示される', async ({ page }) => {
    const rgbValue = page.locator('.ct-value-row').filter({ hasText: 'RGB' }).locator('.ct-value-code');
  ...
  test('ケルビン入力を変更するとHEX値が更新される', async ({ page }) => {
    ...
    // 初期値 (6500K) の HEX を取得
    const hexBefore = await page.locator('.ct-value-row').filter({ hasText: 'HEX' }).locator('.ct-value-code').textContent();

    // 低い温度に変更
    await input.fill('2000');
    await input.press('Tab');

    const hexAfter = await page.locator('.ct-value-row').filter({ hasText: 'HEX' }).locator('.ct-value-code').textContent();
    expect(hexBefore).not.toBe(hexAfter);
  });

  test('HEXコピーボタンが存在する', async ({ page }) => {
    const copyBtn = page.locator('.ct-value-row').filter({ hasText: 'HEX' }).locator('.ct-copy-btn');
