...
test.describe('SSH鍵生成', () => {
  ...
  test('アルゴリズムを切り替えられる', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'RSA 2048' })).toHaveAttribute('aria-pressed', 'true');
    await expect(page.getByRole('button', { name: 'ECDSA P-256' })).toHaveAttribute('aria-pressed', 'false');
  });
