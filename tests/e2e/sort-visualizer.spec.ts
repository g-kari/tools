...
test.describe("ソートアルゴリズム可視化", () => {
  ...
  test("ページが正常に表示される", async ({ page }) => {
    await expect(page.getByRole("heading", { name: "ソートアルゴリズム可視化" })).toBeVisible();
  });

  test("アルゴリズム選択タブが表示される", async ({ page }) => {
    const labels = ["バブルソート", "選択ソート", "挿入ソート", "マージソート", "クイックソート"];
  ...
  test("シャッフルボタンが動作する", async ({ page }) => {
    await expect(page.getByRole("button", { name: /シャッフル/ }).first()).toBeVisible();
    await page
      .getByRole("button", { name: /シャッフル/ })
      .first()
      .click();
  });
