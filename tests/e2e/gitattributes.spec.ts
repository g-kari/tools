...
test.describe('.gitattributes ジェネレーター', () => {
  ...
  test('出力セクションタイトルが表示される', async ({ page }) => {
    await expect(
      page.getByRole('heading', { name: /生成された .gitattributes/ }),
    ).toBeVisible();
  });
  ...
  test('改行コード自動正規化チップが存在する', async ({ page }) => {
    await expect(
      page.getByRole('button', { name: /改行コード自動正規化/ }),
    ).toBeVisible();
  });
  ...
  test('存在しない検索語で「見つかりませんでした」が表示される', async ({ page }) => {
    ...
    await expect(page.getByRole('status')).toContainText('一致するテンプレートが見つかりませんでした');
  });
