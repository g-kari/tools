...
test.describe("SEOメタタグ生成 (/seo-meta)", () => {
  ...
  test("基本SEOタブでタイトルを入力するとコード出力タブに反映される", async ({ page }) => {
  ...
  test("Open Graphタブで入力したタイトルがコードに反映される", async ({ page }) => {
    ...
    await page.getByRole("tab", { name: "コード出力" }).click();
    await expect(page.locator(".seo-meta-output")).toContainText("og:title");
  ...
  test("基本SEOから反映ボタンがOpen GraphタブとTwitterタブに表示される", async ({ page }) => {
    await page.getByRole("tab", { name: "Open Graph" }).click();
    await expect(page.getByRole("button", { name: "基本SEOから反映" })).toBeVisible();
  ...
  test("リセットボタンで入力がクリアされる", async ({ page }) => {
    ...
    await expect(page.getByLabel("ページタイトル入力（推奨60文字以内）")).toHaveValue("");
  });
});
