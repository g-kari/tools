...
test.describe("ひらがな・カタカナ・ローマ字変換 - E2Eテスト", () => {
  ...
  test("デフォルトモードがひらがな→カタカナである", async ({ page }) => {
    const defaultRadio = page.locator('input[name="kana-mode"][value="hiraganaToKatakana"]');
  ...
  test("カタカナ → ひらがな変換が動作する", async ({ page }) => {
    const katakanaRadio = page.locator('input[name="kana-mode"][value="katakanaToHiragana"]');
  ...
  test("仮名 → ローマ字変換が動作する", async ({ page }) => {
    const romajiRadio = page.locator('input[name="kana-mode"][value="kanaToRomaji"]');
