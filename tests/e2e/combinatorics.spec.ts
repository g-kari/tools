...
test.describe("順列・組合せ計算ツール (/combinatorics)", () => {
  ...
  test("ページが正しく表示される", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "順列・組合せ計算ツール" })
    ).toBeVisible();
  });
  ...
  test("プリセットボタンで値がセットされる", async ({ page }) => {
    ...
    await expect(page.locator(".combinatorics-results")).toContainText(
      "2,598,960"
    );
  });
  ...
  test("計算ステップタブが切り替わる", async ({ page }) => {
    ...
    // 組合せタブ（デフォルト）
    await expect(
      page.getByRole("tab", { name: "組合せ nCr の計算手順" })
    ).toHaveAttribute("aria-selected", "true");
    // 順列タブに切替
    await page.getByRole("tab", { name: "順列 nPr の計算手順" }).click();
    await expect(
      page.getByRole("tab", { name: "順列 nPr の計算手順" })
    ).toHaveAttribute("aria-selected", "true");
  });
  ...
  test("n が大きいときパスカルの三角形が非表示になる", async ({ page }) => {
    ...
    await page.getByLabel("r（選ぶ数）").fill("5");
    await expect(
      page.locator(".combinatorics-pascal-container")
    ).not.toBeVisible();
  });
