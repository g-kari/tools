...
test.describe("Number to Words Converter - E2E Tests", () => {
  ...
  test('should convert 21 to English ordinal "twenty-first"', async ({ page }) => {
    const input = page.locator("#nw-input");
    await input.fill("21");
  ...
  test('should convert 300 to Japanese reading "さんびゃく"', async ({ page }) => {
    const input = page.locator("#nw-input");
  ...
  test("should show empty state for ordinal when input is 0", async ({ page }) => {
    const input = page.locator("#nw-input");
    await input.fill("0");
  ...
  test("should show error for non-integer input via button click", async ({ page }) => {
    const input = page.locator("#nw-input");
  ...
  test("should clear the input and results when clear button is clicked", async ({ page }) => {
    const input = page.locator("#nw-input");
    await input.fill("42");

    await expect(page.locator(".nw-result-card").first().locator(".nw-result-value")).toBeVisible();
  ...
  test("should have copy buttons enabled when result exists", async ({ page }) => {
    const input = page.locator("#nw-input");
...
test.describe("Top page - Number Words tool listing", () => {
  ...
  test('should display "数値テキスト変換" in the tool list', async ({ page }) => {
    const link = page.locator('a[href="/number-words"]');
    await expect(link).toBeVisible();
    await expect(link).toContainText("数値テキスト変換");
  });

  test("should navigate to /number-words when clicking the tool card", async ({ page }) => {
    const link = page.locator('a[href="/number-words"]');
    await link.click();
