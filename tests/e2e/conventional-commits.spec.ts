...
test.describe("Conventional Commits バリデーター", () => {
  ...
  test("入力前は空状態が表示される", async ({ page }) => {
    await expect(page.getByText("コミットメッセージを入力してください")).toBeVisible();
  });

  test("有効なコミットメッセージを入力すると ✓ 有効 が表示される", async ({ page }) => {
    await page.getByLabel("コミットメッセージ入力").fill("feat: add new feature");
    await expect(page.getByText("✓ 有効")).toBeVisible();
  });

  test("type が正しくパースされて表示される", async ({ page }) => {
    await page.getByLabel("コミットメッセージ入力").fill("fix(api): handle error");
    await expect(page.getByText("✓ 有効")).toBeVisible();
  ...
  test("BREAKING CHANGE マークが検出される", async ({ page }) => {
    await page.getByLabel("コミットメッセージ入力").fill("feat!: remove endpoint");
  ...
  test("コミットタイプリファレンスが表示される", async ({ page }) => {
    await expect(page.getByText("コミットタイプ リファレンス")).toBeVisible();
