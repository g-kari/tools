...
test.describe('Markdown Preview - E2E Tests', () => {
  ...
  test('should have proper accessibility attributes', async ({ page }) => {
    ...
    // プレビューエリアのrole属性確認
    const previewRegion = page.locator('[role="region"][aria-label="Markdownプレビュー表示エリア"]');
    await expect(previewRegion).toBeVisible();
  });

  test('should navigate to markdown-preview page from other pages via category', async ({ page }) => {
    await page.goto('/');
