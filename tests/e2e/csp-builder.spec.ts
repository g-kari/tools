...
test.describe("CSP ビルダー", () => {
  ...
  test("ソース入力フィールドで値を変更できる", async ({ page }) => {
    ...
    await expect(page.getByRole("region", { name: "生成された CSP" })).toContainText(
      "https://example.com",
    );
  });
  ...
  test("コピーボタンが表示される", async ({ page }) => {
    await expect(
      page.getByRole("button", { name: "CSP ヘッダー値をクリップボードにコピー" }),
    ).toBeVisible();
  });
  ...
  test("既存 CSP をパースしてビルドモードに反映できる", async ({ page }) => {
    ...
    // ビルドモードに戻る
    await expect(page.getByRole("tab", { name: "ビルド" })).toHaveAttribute(
      "aria-selected",
      "true",
    );
  ...
  test("アクセシビリティ: カテゴリセクションにランドマークラベルが設定されている", async ({
    page,
  }) => {
