...
test.describe("CSS Cubic Bezier ジェネレーターページ", () => {
  ...
  test("SVGグラフが表示される", async ({ page }) => {
    await expect(
      page.getByRole("img", { name: /cubic-bezier/ })
    ).toBeVisible();
  });
  ...
  test("アニメーションプレビューが表示される", async ({ page }) => {
    await expect(
      page.getByLabel("タイミング関数のアニメーションプレビュー")
    ).toBeVisible();
  });
  ...
  test("再生ボタンが表示される", async ({ page }) => {
    await expect(
      page.getByRole("button", { name: "アニメーションを再生" })
    ).toBeVisible();
  });
  ...
  test("linear プリセットをクリックするとCSS値が更新される", async ({
    page,
  }) => {
  ...
  test("ease プリセットをクリックするとボタンが選択状態になる", async ({
    page,
  }) => {
