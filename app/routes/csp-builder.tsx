  ...
  head: () => ({
    meta: [
      ...
      { name: "twitter:title", content: "CSP ビルダー | Web ツール集" },
      {
        ...
        content: "Content-Security-Policy ヘッダーをGUIで構築するツール。",
      },
    ],
  }),
    ...
    <div className={`csp-directive-row ${!entry.enabled ? "csp-directive-row-disabled" : ""}`}>
      <div className="csp-directive-row-header">
        ...
        <div className="csp-directive-name-wrap">
          ...
          {info?.deprecated && (
            <span className="csp-deprecated-badge" title="非推奨">
              非推奨
            </span>
          )}
        </div>
      ...
      {info && <p className="csp-directive-description">{info.description}</p>}
...
function CspBuilderPage() {
  ...
  const handleUpdateSources = useCallback((name: string, sourcesStr: string) => {
    ...
    setDirectives((prev) => prev.map((d) => (d.name === name ? { ...d, sources } : d)));
  }, []);
  ...
  const handleAddDirective = useCallback(() => {
    ...
    setDirectives((prev) => [...prev, { name, sources: [], enabled: true }]);
    setAddDirectiveName("");
        ...
        {mode === "parse" && (
          <div className="csp-parse-section">
            ...
            <textarea
              ...
              placeholder={
                "例: default-src 'self'; script-src 'self' https://cdn.example.com; img-src 'self' data:"
              }
              rows={5}
              spellCheck={false}
              autoComplete="off"
          ...
          <>
            ...
            <section className="csp-category-section" aria-labelledby="csp-cat-fetch">
              ...
              <div className="csp-directives-list">
                {directives
                  .filter((d) => CSP_FETCH_DIRECTIVES.some((f) => f.name === d.name))
                  .map((entry) => (
                    <DirectiveRow
                      key={entry.name}
                      entry={entry}
                ...
                {directives.filter((d) => CSP_FETCH_DIRECTIVES.some((f) => f.name === d.name))
                  .length === 0 && (
            ...
            <section className="csp-category-section" aria-labelledby="csp-cat-doc">
              ...
              <div className="csp-directives-list">
                {directives
                  .filter((d) => CSP_DOCUMENT_DIRECTIVES.some((f) => f.name === d.name))
                  .map((entry) => (
                    <DirectiveRow
                      key={entry.name}
                      entry={entry}
                ...
                {directives.filter((d) => CSP_DOCUMENT_DIRECTIVES.some((f) => f.name === d.name))
                  .length === 0 && (
            ...
            <section className="csp-category-section" aria-labelledby="csp-cat-other">
              ...
              <div className="csp-directives-list">
                {directives
                  .filter((d) => CSP_OTHER_DIRECTIVES.some((f) => f.name === d.name))
                  .map((entry) => (
                    <DirectiveRow
                      key={entry.name}
                      entry={entry}
                ...
                {directives.filter((d) => CSP_OTHER_DIRECTIVES.some((f) => f.name === d.name))
                  .length === 0 && (
            ...
            {/* カスタムディレクティブ */}
            {directives.filter((d) => !ALL_CSP_DIRECTIVES.some((def) => def.name === d.name))
              .length > 0 && (
              <section className="csp-category-section" aria-labelledby="csp-cat-custom">
                ...
                <div className="csp-directives-list">
                  {directives
                    .filter((d) => !ALL_CSP_DIRECTIVES.some((def) => def.name === d.name))
                    .map((entry) => (
                      <DirectiveRow
                        key={entry.name}
                        entry={entry}
          ...
          <div className="csp-output-section">
            ...
            {cspString && (
              <div className="csp-http-example">
                ...
                <code className="csp-output-code">Content-Security-Policy: {cspString}</code>
              </div>
            )}
          </div>
        )}
