...
function WordleGame() {
  ...
  const [allResults, setAllResults] = useState<Exclude<LetterState, "empty" | "input">[][]>([]);
  ...
  }, [gameStatus, currentGuess, guesses, allResults, targetWord, announceStatus]);
    ...
    <>
      <div className="tool-container">
        ...
        <div className="wordle-grid-container">
          <div className="wordle-grid" role="grid" aria-label="Wordleグリッド">
            {rows.map((row, rowIdx) => (
              <div
                key={rowIdx}
          ...
          >
            {gameStatus === "won" ? (
              <>
                ...
                <span>
                  クリア！正解は <strong>{targetWord}</strong>
                </span>
              </>
            ) : (
              <>
                <span className="wordle-result-emoji">💀</span>
                <span>
                  ゲームオーバー。正解は <strong>{targetWord}</strong>
                </span>
              </>
            )}
          </div>
        ...
        {/* オンスクリーンキーボード */}
        <div className="converter-section">
          <div className="wordle-keyboard" role="group" aria-label="文字を入力">
