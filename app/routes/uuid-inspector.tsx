...
import { parseUUID, UUID_SAMPLES, UUID_NIL, type UUIDInfo } from "~/utils/uuid-inspector";
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "UUID インスペクター | Web ツール集" },
      {
        ...
        content:
          "UUID の構造を解析。バージョン (v1〜v8) 判定・タイムスタンプ抽出・バイナリ表示。RFC 4122 / RFC 9562 準拠。",
      },
    ...
    <>
      <div className="tool-container">
        ...
        <div className="uuid-samples" role="group" aria-label="サンプル UUID">
          <span className="uuid-samples-label" aria-hidden="true">
            サンプル:
          </span>
          {UUID_SAMPLES.map((s) => (
            <button
        ...
        {/* 結果 */}
        {info && info.valid && <UUIDResult info={info} onCopy={handleCopy} />}
    ...
    <section className="uuid-result-section" aria-label="UUID 解析結果">
      ...
      {!isSpecial && (
        <div className="uuid-meta-grid">
          ...
          {info.unixMs !== undefined && <UUIDRow label="Unix ms" value={info.unixMs.toString()} />}
          ...
          {info.macAddress && <UUIDRow label="MAC アドレス" value={info.macAddress} />}
        </div>
      )}
      ...
      {info.components && (
        <details className="uuid-fields-section">
          ...
          <div className="uuid-fields-grid">
            <FieldRow
              label="time_low"
              value={info.components.timeLow}
              bits="ビット 0-31"
              color="f1"
            />
            <FieldRow
              label="time_mid"
              value={info.components.timeMid}
              bits="ビット 32-47"
              color="f2"
            />
            <FieldRow
              label="time_hi_version"
              value={info.components.timeHiAndVersion}
              bits="ビット 48-63"
              color="f3"
            />
            <FieldRow
              label="clk_seq_hi"
              value={info.components.clockSeqHiRes}
              bits="ビット 64-71"
              color="f4"
            />
            <FieldRow
              label="clk_seq_low"
              value={info.components.clockSeqLow}
              bits="ビット 72-79"
              color="f4"
            />
