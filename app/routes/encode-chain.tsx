...
/** 1ステップの変換を適用する */
export function applyTransform(input: string, type: TransformType): { output: string; error?: string } {
  try {
    switch (type) {
      case 'base64-encode':
  ...
  return (
    <div className="tool-container">
      ...
      <section className="ec-section" aria-labelledby="ec-input-heading">
        <h2 id="ec-input-heading" className="ec-section-title">入力テキスト</h2>
        <textarea
      ...
      <section className="ec-section" aria-labelledby="ec-add-heading">
        <h2 id="ec-add-heading" className="ec-section-title">変換ステップを追加</h2>
        <div className="ec-add-row">
          ...
          <button type="button" className="btn-primary" onClick={handleAddStep} aria-label="ステップを追加">
            + 追加
          </button>
          {steps.length > 0 && (
            <button
      ...
      {steps.length === 0 ? (
        ...
        <section className="ec-chain" aria-label="変換チェーン">
          ...
          {results.map((result, index) => {
            ...
            return (
              <div key={result.step.id} className={`ec-step ${result.error ? 'ec-step-error' : ''}`}>
                {/* ステップヘッダー */}
                <div className="ec-step-header">
                  ...
                  <select
                    ...
                    onChange={(e) => handleChangeType(result.step.id, e.target.value as TransformType)}
                ...
                {/* 出力バッジ */}
                <div className={`ec-step-io ${result.error ? 'ec-step-io-error' : 'ec-step-io-output'}`}>
