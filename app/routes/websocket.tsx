  ...
  head: () => ({
    meta: [
      { title: 'WebSocket テスター | Web ツール集' },
      {
        name: 'description',
        content:
          'WebSocketエンドポイントへの接続・メッセージ送受信テストツール。接続状態のモニタリング・メッセージログ・JSON整形表示に対応。ブラウザ内完結。',
      },
...
/** メッセージタイプセレクターの種別 */
type InputMessageType = 'text' | 'json';
 ...
 * JSON文字列のパースを試みる
 * @param str - パース対象の文字列
 * @returns パース成功時はインデントされたJSON文字列、失敗時は元の文字列
 */
 ...
 * @returns 有効なws://またはwss://URLであればtrue
 */
...
function WebSocketTool() {
  ...
  /** ログにエントリを追加する */
  const addLog = useCallback((type: MessageType, content: string) => {
  ...
  const handleConnect = useCallback(() => {
    ...
    try {
      ...
      ws.onmessage = (event) => {
        const content = typeof event.data === 'string' ? parseJsonSafe(event.data) : String(event.data);
  ...
  const handleSend = useCallback(() => {
    ...
    if (inputType === 'json') {
      try {
        toSend = JSON.stringify(JSON.parse(message));
      } catch {
    ...
    const displayContent = inputType === 'json' ? parseJsonSafe(toSend) : toSend;
    addLog('sent', displayContent);
    setMessage('');
    ...
    <>
      <div className="tool-container">
        ...
        <div className="converter-section">
          ...
          {/* メッセージタイプ選択 */}
          <div className="websocket-message-type-selector" role="group" aria-label="メッセージタイプ">
            <button
              type="button"
              className={`hash-input-tab${inputType === 'text' ? ' active' : ''}`}
            ...
            <button
              type="button"
              className={`hash-input-tab${inputType === 'json' ? ' active' : ''}`}
              onClick={() => setInputType('json')}
              aria-pressed={inputType === 'json'}
            >
              JSON
            </button>
          </div>
          ...
          <textarea
            ...
            placeholder={inputType === 'json' ? '{"key": "value"}' : 'メッセージを入力...'}
            rows={4}
          ...
          sections={[
            {
              ...
              items: [
                ...
                '接続後、テキストまたはJSONメッセージを送信できます',
                'Ctrl+Enterでも送信できます',
                '受信したメッセージはログエリアにリアルタイム表示されます',
                '有効なJSONを受信した場合は自動的に整形表示されます',
              ],
            },
            {
              title: 'テスト用エコーサーバー',
            ...
            },
            {
              ...
              items: [
                ...
                '受信メッセージが有効なJSONの場合は自動的に整形表示されます',
              ],
            },
          ]}
        />
