  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Base62エンコード・デコード | Web ツール集" },
      {
        ...
        content:
          "Base62エンコード・デコード。URL 短縮・短い一意 ID 生成に使われる英数字のみのエンコード方式。",
      },
      ...
      <div className="tool-container">
        ...
        {errorMessage && input.trim() && (
          <div className="b62-error" role="alert" aria-label="エラー">
            <span className="b62-error-icon" aria-hidden="true">
              ⚠
            </span>
            {errorMessage}
          </div>
          ...
          <div className="converter-section">
            <div className="b62-output-header">
              ...
              {inputMode === "text" && mode === "decode" && decodeResult && (
                <span className="b62-output-meta">{decodeResult.bytes.length} バイト</span>
              )}
            </div>
            {inputMode === "text" ? (
              <textarea
