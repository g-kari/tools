...
import { validateIsbn, SAMPLE_ISBNS, type IsbnResult } from "../utils/isbn";
  ...
  head: () => ({
    meta: [
      ...
      },
      {
        ...
        content: "ISBN-10 / ISBN-13 の検証・相互変換。チェックデジット計算・フォーマット表示対応。",
      },
    ],
  }),
  component: IsbnTool,
...
function IsbnTool() {
  ...
  const showError = showResult && !result.isValid && result.raw.length >= 10;
      ...
      <div className="isbn-container">
        ...
        {showResult && (result.isValid || result.isValidLength) && (
          <div className="isbn-result-section" aria-live="polite">
            <div className={`isbn-result-badge ${result.isValid ? "valid" : "invalid"}`}>
            ...
            <div className="isbn-result-grid">
              ...
              <ResultField
                ...
                onCopy={handleCopy}
              />
              <ResultField label="ISBN-10" value={result.isbn10} onCopy={handleCopy} />
              <ResultField label="ISBN-13" value={result.isbn13} onCopy={handleCopy} />
            </div>
          </div>
        )}
