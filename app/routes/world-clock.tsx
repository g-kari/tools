  ...
  head: () => ({
    meta: [
      ...
      { property: 'og:title', content: 'ワールドクロック | Web ツール集' },
      {
        ...
        content:
          '世界の主要都市の現在時刻をリアルタイム表示。25都市対応・12h/24h切替。',
      },
  ...
  return (
    <>
      <div className="tool-container">
        ...
        {/* 都市セレクターパネル */}
        {showSelector && (
          <div id="wc-selector" className="wc-selector-panel" role="region" aria-label="表示都市の選択">
            <div className="wc-selector-header">
              <span className="wc-selector-title">表示する都市</span>
              <div className="wc-selector-actions">
                <button
                  type="button"
                  className="wc-text-btn"
                  onClick={handleSelectAll}
                >
                  すべて選択
                </button>
                <span className="wc-text-sep" aria-hidden="true">|</span>
                <button
                  type="button"
                  className="wc-text-btn"
            ...
            <div
              className="wc-city-checkboxes"
              role="group"
              aria-label="都市チェックボックス一覧"
            >
              {TIMEZONES.map((tz) => (
