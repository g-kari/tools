  ...
  head: () => ({
    meta: [
      ...
      },
      {
        ...
        content: "テキストを UTF-8 バイト列の2進数に変換するツール。日本語・絵文字にも対応。",
      },
    ],
  }),
...
function TextBinaryConverter() {
  ...
  const showAutoDetectHint = autoDetectedMode !== null && autoDetectedMode !== mode;
  ...
  const decodeError = decodeResult && !decodeResult.success ? decodeResult.error : null;
    ...
    <>
      <div className="text-binary-container">
        ...
        <section aria-labelledby="text-binary-heading">
          ...
          <div className="text-binary-mode-buttons" role="group" aria-label="変換モード選択">
            <button
        ...
        <div className="text-binary-io-section">
          ...
          {showAutoDetectHint && (
            <p className="text-binary-hint" id="text-binary-input-hint">
              入力から{autoDetectedMode === "binary-to-text" ? "バイナリ" : "テキスト"}
              を検出しました。
              <button type="button" className="link-button" onClick={handleAutoDetect}>
                モードを切り替える
              </button>
            </p>
          )}
          ...
          <div className="text-binary-io-section">
            ...
            {mode === "text-to-binary" && encodeResult && (
              <div className="text-binary-stats" aria-label="統計情報">
                <span className="text-binary-stat-badge">文字数: {encodeResult.charCount}</span>
                <span className="text-binary-stat-badge">バイト数: {encodeResult.inputBytes}</span>
                <span className="text-binary-stat-badge">
                  ビット数: {encodeResult.inputBytes * 8}
                </span>
              </div>
            )}
            {mode === "binary-to-text" && decodeResult?.success && (
              <div className="text-binary-stats" aria-label="統計情報">
                <span className="text-binary-stat-badge">
        ...
        {mode === "text-to-binary" && encodeResult && encodeResult.byteBreakdown.length > 0 && (
          <div className="text-binary-breakdown-section">
            <h3 className="text-binary-breakdown-title">バイト内訳</h3>
            <table className="text-binary-breakdown-table" aria-label="文字ごとのバイト内訳">
              <thead>
                <tr>
                  <th scope="col">文字</th>
              ...
              <tbody>
                {encodeResult.byteBreakdown.map((info, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="text-binary-hex-cell">{info.hexBytes.join(" ")}</td>
