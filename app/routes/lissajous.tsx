  ...
  head: () => ({
    meta: [
      ...
      },
      {
        ...
        content: "リサジュー図形をアニメーションで描画。周波数比・位相差を変えて様々な図形を探索。",
      },
        ...
        <div className="lissajous__control-row">
          <div className="lissajous__control">
            ...
            <input
              ...
              step={1}
              value={params.freqX}
              onChange={(e) => setParams((p) => ({ ...p, freqX: Number(e.target.value) }))}
              aria-label="X軸の周波数"
            />
          </div>
          <div className="lissajous__control">
            ...
            <input
              ...
              step={1}
              value={params.freqY}
              onChange={(e) => setParams((p) => ({ ...p, freqY: Number(e.target.value) }))}
              aria-label="Y軸の周波数"
            />
          </div>
        </div>
        ...
        <div className="lissajous__control-row">
          <div className="lissajous__control">
            ...
            <input
              ...
              onChange={(e) => setParams((p) => ({ ...p, phaseDeg: Number(e.target.value) }))}
              aria-label="位相差"
            />
          </div>
          <div className="lissajous__control">
            <label className="lissajous__label">
              <span>振幅</span>
              <span className="lissajous__label-value">{Math.round(params.amplitude * 100)}%</span>
            </label>
            <input
              type="range"
              className="lissajous__slider"
              ...
              onChange={(e) => setParams((p) => ({ ...p, amplitude: Number(e.target.value) }))}
              aria-label="振幅"
            />
          </div>
        </div>
