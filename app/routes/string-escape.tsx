...
import { ESCAPE_MODES, escapeString, unescapeString, type EscapeMode } from "~/utils/string-escape";
...
function StringEscapeTool() {
  ...
  const handleModeChange = useCallback(
    (newMode: EscapeMode) => {
      setMode(newMode);
      announceStatus(
        `モードを${ESCAPE_MODES.find((m) => m.id === newMode)?.label ?? newMode}に変更しました`,
      );
    },
    [announceStatus],
  );
  ...
  return (
    <>
      <div className="tool-container">
        {/* モード選択 */}
        <div className="string-escape-modes" role="group" aria-label="エスケープモード選択">
          {ESCAPE_MODES.map((m) => (
            <button
              key={m.id}
              type="button"
        ...
        <div className="string-escape-layout">
          ...
          <div>
            <div className="string-escape-section-header">
              ...
              <span
                className="string-escape-char-count"
                aria-label={`入力文字数: ${rawInput.length}`}
              >
          ...
          <div>
            <div className="string-escape-section-header">
              ...
              <div className="string-escape-header-actions">
                ...
                <span
                  className="string-escape-char-count"
                  aria-label={`出力文字数: ${escapedOutput.length}`}
                >
