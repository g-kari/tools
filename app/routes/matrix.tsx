  ...
  return (
    <div className="matrix-grid-wrapper">
      ...
      <div className="matrix-brackets">
        <span className="matrix-bracket-open" aria-hidden="true">
          [
        </span>
        <table className="matrix-table" role="grid" aria-label={label ?? "行列"}>
          <tbody>
            {matrix.map((row, i) => (
              <tr key={i}>
                {row.map((v, j) => (
        ...
        <span className="matrix-bracket-close" aria-hidden="true">
          ]
        </span>
      </div>
    </div>
...
function MatrixPage() {
  ...
  return (
    <div className="tool-container">
      ...
      <div className="matrix-inputs">
        ...
        {mode === "binary" && (
          <div className="matrix-op-divider" aria-hidden="true">
            <span className="matrix-op-divider-symbol">{currentOp?.symbol ?? "○"}</span>
          </div>
        )}
      ...
      <div className="matrix-result-section" aria-live="polite">
        <div className="matrix-result-header">
          <span className="matrix-result-title">
            計算結果: <span className="matrix-result-op">{currentOp?.symbol}</span>
          </span>
        </div>

        {!result && <div className="matrix-result-empty">行列を入力すると結果が表示されます</div>}
          ...
          <div className="matrix-result-content">
            ...
            {result.scalar !== null && (
              <div className="matrix-scalar-result">
                <span className="matrix-scalar-result-value">{formatMatrixNum(result.scalar)}</span>
              </div>
            )}
          </div>
        )}
      ...
      {/* 入力行列プレビュー */}
      {(parsedA.matrix || (mode === "binary" && !showScalarInput && parsedB.matrix)) && (
        <div className="matrix-preview-section">
          <p className="matrix-preview-title">入力行列プレビュー</p>
          <div className="matrix-preview-row">
            {parsedA.matrix && <MatrixGrid matrix={parsedA.matrix} label="行列 A" />}
        ...
        sections={[
          {
            title: "Tips",
            items: [
              "スペース・タブ・カンマで列を区切り、改行で行を区切って入力",
              "行列式・逆行列・トレースは正方行列のみ対応",
              "乗算は A の列数と B の行数が一致する必要があります（A: m×n、B: n×p → 結果: m×p）",
              "負数・小数も使用可能（例: -1.5 や 3.14）",
              "逆行列が存在しない場合（特異行列）はエラーが表示されます",
            ],
          },
        ]}
      />
