...
export const REGEX_LIBRARY: RegexEntry[] = [
  ...
  },
  {
    ...
    flags: '',
    description: 'RFC 5321 に準拠したより厳密なメールアドレス検証パターン。ドメインの先頭ハイフンを禁止。',
    category: 'email',
    examples: {
  ...
  },
  {
    ...
    category: 'url',
    examples: {
      match: ['https://example.com', 'ftp://files.example.org/file.txt', 'ssh://user@host.example.com'],
  ...
  },
  {
    ...
    flags: '',
    description: '192.168.0.0/24 のような CIDR 表記の IPv4 ネットワークアドレスに一致するパターン。',
    category: 'network',
    examples: {
  ...
  // ─── 電話番号 ─────────────────────────────────────────────
  {
    ...
    flags: '',
    description: '日本の携帯電話番号（070/080/090 始まり）に一致するパターン。ハイフン・スペースありなし両対応。',
    category: 'phone',
    examples: {
  ...
  },
  {
    ...
    flags: '',
    description: '日本の固定電話番号に一致するパターン。ハイフン・スペース区切りに対応（市外局番2〜5桁）。',
    category: 'phone',
    examples: {
  ...
  },
  {
    ...
    flags: '',
    description: '英字始まり、英数字とアンダースコアのみで 3〜20 文字のユーザー名に一致するパターン。',
    category: 'code',
    examples: {
  ...
  },
  {
    ...
    pattern: '^\\s+$',
    flags: '',
    description: '1文字以上の空白文字（スペース・タブ・改行等）のみで構成されるテキストに一致するパターン。',
    category: 'text',
    examples: {
      match: [' ', '\t', '   '],
      noMatch: ['text', ' a ', ''],
  ...
  },
  {
    ...
    pattern: '\\r\\n|\\r|\\n',
    flags: 'g',
    description: 'Windows (CRLF)・Mac 旧形式 (CR)・Unix (LF) の改行文字すべてに一致するパターン。テキスト正規化に使用。',
    category: 'text',
    examples: {
      match: ['\r\n', '\n', '\r'],
  ...
  // ─── 金融 ─────────────────────────────────────────────────
  {
    ...
    flags: '',
    description: '16桁のクレジットカード番号（スペース・ハイフンありなし両対応）に一致するパターン。',
    category: 'finance',
    examples: {
  ...
  },
  {
    ...
    pattern: '^5[1-5][0-9]{14}$|^2(?:2[2-9][1-9]|2[3-9][0-9]|[3-6][0-9]{2}|7[01][0-9]|720)[0-9]{12}$',
    flags: '',
