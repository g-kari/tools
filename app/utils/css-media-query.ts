...
export function generateMediaQueryCSS(rule: MediaQueryRule): string {
  ...
  const innerLines = rule.innerCSS
    ...
    .map((l) => `    ${l.trimStart()}`)
    .join("\n");
  const inner = rule.innerCSS.trim() ? innerLines : "    /* ここにスタイルを記述 */";
  return `${query} {\n  ${selector} {\n${inner}\n  }\n}`;
}
 ...
 * @returns フォーマット済み文字列
 */
export function formatMediaQueryOutput(rule: MediaQueryRule, outputType: OutputType): string {
  if (outputType === "json") {
    const obj = {
 ...
 */
export function checkMediaQueryMatch(rule: MediaQueryRule, previewWidthPx: number): boolean {
