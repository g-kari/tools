...
export function generateNginxConfig(config: NginxServerConfig): string {
  ...
  const serverName = domains.length > 0 ? domains.join(" ") : "example.com";
  const lines: string[] = [];
  ...
  if (hasHeaders) {
    ...
    if (headers.xFrameOptions !== "none") {
      lines.push(`    add_header X-Frame-Options "${headers.xFrameOptions}" always;`);
    }
    ...
    if (headers.referrerPolicy !== "") {
      lines.push(`    add_header Referrer-Policy "${headers.referrerPolicy}" always;`);
    }
  }
